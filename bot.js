...
exports.start = function(SETUP) {
  ...
  const UpdateEmbed = function() {
    let dot = TICK_N % 2 === 0 ? 'Queue' : 'CARS AND RP';
    let embed = new Discord.RichEmbed()
    .setAuthor("PARADISE CAR CLUB BOT STATUS", "https://cdn.discordapp.com/attachments/915996073435230294/934803300837126184/digital-digital-art-artwork-illustration-drawing-hd-wallpaper-preview.png")
    .setColor(0x2894C2)
    .setFooter(TICK_N % 2 === 0 ? '⚪ status' : '⚫ status')
    .setTimestamp(new Date())
    .addField('\n\u200b\nHow can you join the server?','You can join the server by going to FiveM and searching up: PARADISE CAR CLUB or press F8 and paste **connect 185.249.199.123:30388**. \n\u200b\n',false)
    if (STATUS !== undefined)
    {
  ...
  const updateMessage = function() {
    getVars().then((vars) => {
      getPlayers().then((players) => {
        ...
        .addField('PCC',queue === 'Enabled' || queue === undefined ? '0' : queue.split(':')[1].trim(),true)
  ...
  bot.on('ready',() => {
    ...
    bot.user.setActivity('PCC',{'url':'https://www.youtube.com/channel/UCtFzCJpM40lfZ2hUKU6rjOw?view_as=subscriber','type':'STREAMING'});
  ...
  bot.on('message', async function (msg) {
    if (msg.channel.id === '934887484544921601') {
        await msg.react(bot.emojis.get('934887484544921601'));
        await msg.react(bot.emojis.get('934887484544921601'));
    }
});
